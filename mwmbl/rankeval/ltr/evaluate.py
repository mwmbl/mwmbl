...
from mwmbl.tinysearchengine.ltr import FeatureExtractor, RankingPredictor, RustXGBPipeline, ThresholdPredictor
...
PREDICTORS = {
    ...
    "xgb": make_pipeline(
        FeatureExtractor(), ThresholdPredictor(0.0, XGBClassifier(scale_pos_weight=0.1, reg_lambda=2))
    ),
    "xgb_rust": RustXGBPipeline(threshold=0.0, scale_pos_weight=0.1, reg_lambda=2.0, num_rounds=100),
    "xgb_limit_terms": RankingPredictor(
        FeatureExtractor(), ThresholdPredictor(0.0, XGBClassifier(scale_pos_weight=0.1, reg_lambda=2))
    ),
...
def run():
    ...
    predictors = {
        ...
        "xgb_limit_terms": RankingPredictor(
            FeatureExtractor(), ThresholdPredictor(0.0, XGBClassifier(**xgb_classifier_params))
        ),
        "xgb_regressor": make_pipeline(
            FeatureExtractor(), XGBRegressor(objective="reg:pseudohubererror", **xgb_params)
        ),
