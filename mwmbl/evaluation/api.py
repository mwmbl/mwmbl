...
from .schemas import EvaluationJobResponse, EvaluationResultsResponse, WasmSubmissionResponse
...
def submit_wasm(request, file: UploadedFile = File(...)):
    ...
    job = WasmEvaluationJob.objects.create(user=request.user, wasm_file=wasm_bytes, status="VALIDATED")

    return WasmSubmissionResponse(
        job_id=job.id, status=job.status, message="WASM file validated and job created successfully"
    )
...
def run_evaluation(request, job_id: int):
    ...
    try:
        ...
        return EvaluationResultsResponse(job_id=job.id, status=job.status, results=job.results)
        ...
        return EvaluationResultsResponse(job_id=job.id, status=job.status, error_message=job.error_message)
...
def get_evaluation_results(request, job_id: int):
    ...
    return EvaluationResultsResponse(
        job_id=job.id, status=job.status, results=job.results, error_message=job.error_message
    )
