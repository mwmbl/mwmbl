...
def test_crawl_date_is_the_crawl_time_not_the_start_of_the_month(clean_bloom_filters):
    ...
    crawled_at = datetime(2026, 8, 31, 12, 0, 0, tzinfo=timezone.utc)
    found = FoundURL(
...
def test_crawl_date_from_a_real_batch_timestamp_is_aware(clean_bloom_filters):
    """Batch timestamps are timezone-aware, and so is the clock queue_urls subtracts
    last_crawled from: mixing an aware and a naive datetime raises."""
    ...
    assert new_urls[0].last_crawled.tzinfo == timezone.utc
    assert new_urls[0].last_crawled == crawled_at
