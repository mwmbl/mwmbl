...
BAD = Document(
    title="Bad page about apples",
    url="https://badsite.test/apples",
    extract="apples and oranges and bananas",
    score=1.0,
)
GOOD = Document(
    title="Good page about apples",
    url="https://example.test/apples",
    extract="apples and oranges and bananas",
    score=1.0,
)
...
def test_purge_removes_only_the_documents_that_are_still_blacklisted(index_path):
    ...
    with TinyIndex(Document, index_path, "w") as tiny_index:
        removed = purge_documents(tiny_index, [BAD, GOOD], is_blacklisted=lambda domain: domain == "badsite.test")
...
def test_purge_removes_the_copy_filed_under_a_query_term(index_path):
    ...
    wordy = Document(
        title="Bad page about apples",
        url="https://badsite.test/apples",
        extract="apples and oranges and bananas here are many more words including zebra",
        score=1.0,
    )
    ...
    queued = Document(title=wordy.title, url=wordy.url, extract=wordy.extract, score=wordy.score, term="zebra")
    with TinyIndex(Document, index_path, "w") as tiny_index:
...
def test_pages_for_document_includes_the_term_page(index_path):
    with TinyIndex(Document, index_path, "r") as tiny_index:
        ...
        with_term = pages_for_document(
            tiny_index, Document(title=BAD.title, url=BAD.url, extract=BAD.extract, score=BAD.score, term="wibble")
        )
