...
from mwmbl.tinysearchengine.indexer import METADATA_SIZE, Document, DocumentState, PageError, TinyIndex
...
def _try_lock_in_child(index_path: str, page: int, queue):
    ...
    with TinyIndex(Document, index_path, "w") as indexer:
        ...
        try:
            fcntl.fcntl(
                indexer.index_file.fileno(),
                F_OFD_SETLK,
                struct.pack(_FLOCK_STRUCT, fcntl.F_WRLCK, os.SEEK_SET, start, indexer.page_size, 0),
            )
            queue.put(True)
        except OSError:
            queue.put(False)
    ...
    with TinyIndex(Document, index_path, "w") as indexer:
        with indexer.page(target) as page:
            ...
            with TinyIndex(Document, index_path, "r"):
                pass
            assert not child_can_lock(), "closing an unrelated fd released the lock - this needs an OFD lock"
...
def test_page_writes_nothing_unless_store_is_called(index_path):
    seeded = [Document("Zebra", "https://en.wikipedia.org/wiki/Zebra", "", 3.0, "zebra", state=DocumentState.FROM_WIKI)]
    with TinyIndex(Document, index_path, "w") as indexer:
        target = indexer.get_key_page_index("zebra")
...
def test_store_reports_how_many_documents_actually_fit(index_path):
    ...
    many = [
        Document(
            f"Zebra {i}",
            f"https://en.wikipedia.org/wiki/Zebra_{i}",
...
def _fail_to_lock(monkeypatch, err: OSError):
    ...
    monkeypatch.setattr(indexer_module, "_set_page_lock", lambda *args: (_ for _ in ()).throw(err))
...
def _store_repeatedly(index_path: str, term: str, url_prefix: str, count: int):
    ...
    for i in range(count):
        ...
        index_results_against_query(
            [Document(f"Zebra {url_prefix} {i}", f"https://example.com/{url_prefix}/{i}", "", 3.0, None)],
            term,
            index_path,
        )
...
def test_concurrent_writers_leave_the_page_readable(index_path):
    ...
    context = multiprocessing.get_context("fork")
    workers = [context.Process(target=_store_repeatedly, args=(index_path, "zebra", f"w{n}", 30)) for n in range(4)]
...
def test_purging_skips_a_page_it_cannot_read(index_path):
    ...
    with TinyIndex(Document, index_path, "w") as indexer:
        pages = {indexer.get_key_page_index(term) for term in ("spam", "extract", "spam example com")}
