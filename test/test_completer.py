...
def mockCompleterData(mocker, data):
    ...
    mocker.patch("mwmbl.tinysearchengine.completer.Completer.get_terms", return_value=testDataFrame)


def test_correctCompletions(mocker):
    # Mock completer with custom data
    testdata = [[0, "build", 4], [1, "builder", 3], [2, "announce", 2], [3, "buildings", 1]]
...
def test_correctSortOrder(mocker):
    # Mock completer with custom data
    testdata = [[0, "build", 4], [1, "builder", 1], [2, "announce", 2], [3, "buildings", 3]]
    mockCompleterData(mocker, testdata)
...
def test_idempotencyWithSameScoreCompletions(mocker):
    # Mock completer with custom data
    testdata = [[0, "build", 1], [1, "builder", 1], [2, "announce", 1], [3, "buildings", 1]]
    mockCompleterData(mocker, testdata)
