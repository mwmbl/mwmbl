...
def user2():
    ...
    return User.objects.create_user(username="testuser2", email="test2@example.com", password="testpass123")
...
def verified_user2(user2):
    ...
    EmailAddress.objects.create(user=user2, email="test2@example.com", verified=True, primary=True)
    return user2
...
def sample_vote_data():
    ...
    return {"url": "https://example.com/test-page", "query": "test search query", "vote_type": "upvote"}
...
def test_vote_on_search_result_success(client, verified_user, access_token, sample_vote_data):
    ...
    vote = SearchResultVote.objects.get(
        user=verified_user, url=sample_vote_data["url"], query=sample_vote_data["query"]
    )
...
def test_vote_update_existing_vote(client, verified_user, access_token, sample_vote_data):
    ...
    SearchResultVote.objects.create(
        user=verified_user, url=sample_vote_data["url"], query=sample_vote_data["query"], vote_type="upvote"
    )
...
def test_vote_without_authentication(client, sample_vote_data):
    ...
    response = client.post(
        "/api/v1/platform/search-results/vote", data=json.dumps(sample_vote_data), content_type="application/json"
    )
...
def test_get_vote_counts_single_url(client, verified_user, verified_user2, access_token, sample_vote_data):
    ...
    SearchResultVote.objects.create(
        user=verified_user2, url=sample_vote_data["url"], query=sample_vote_data["query"], vote_type="downvote"
    )

    vote_stats_request = {"query": sample_vote_data["query"], "urls": [sample_vote_data["url"]]}
...
def test_get_vote_counts_multiple_urls(client, verified_user, access_token):
    ...
    SearchResultVote.objects.create(user=verified_user, url=url1, query=query, vote_type="upvote")
    SearchResultVote.objects.create(user=verified_user, url=url2, query=query, vote_type="downvote")

    vote_stats_request = {"query": query, "urls": [url1, url2]}
...
def test_get_vote_counts_no_urls(client, access_token):
    ...
    vote_stats_request = {"query": "test", "urls": []}
...
def test_remove_vote_success(client, verified_user, access_token, sample_vote_data):
    ...
    response = client.delete(
        "/api/v1/platform/search-results/vote",
        data=json.dumps({"url": sample_vote_data["url"], "query": sample_vote_data["query"]}),
        content_type="application/json",
        HTTP_AUTHORIZATION=f"Bearer {access_token}",
    )
...
def test_get_user_vote_history(client, verified_user, access_token):
    ...
    for vote_data in votes_data:
        SearchResultVote.objects.create(user=verified_user, **vote_data)

    response = client.get("/api/v1/platform/search-results/my-votes", HTTP_AUTHORIZATION=f"Bearer {access_token}")
...
def test_vote_unique_constraint(verified_user):
    ...
    vote_data = {"user": verified_user, "url": "https://example.com/test", "query": "test query", "vote_type": "upvote"}
    ...
    with transaction.atomic():
        count = SearchResultVote.objects.filter(
            user=verified_user, url=vote_data["url"], query=vote_data["query"]
        ).count()
        assert count == 1
