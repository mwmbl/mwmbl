...
def test_large_url_list_post_method(client, verified_user, access_token):
    ...
    vote_stats_request = {"query": query, "urls": urls}
...
def test_very_long_urls_in_large_list(client, verified_user, access_token):
    ...
    for i in range(50):
        ...
        if i % 3 == 0:
            SearchResultVote.objects.create(user=verified_user, url=url, query=query, vote_type="upvote")

    # Add the original problematic URLs from the bug report
    problematic_urls = ["http://keybase.io/hello", "http://keybase.io/hello_", "https://github.com/hello"]
    base_urls.extend(problematic_urls)

    vote_stats_request = {"query": query, "urls": base_urls}
...
def test_edge_case_empty_and_malformed_urls(client, verified_user, access_token):
    ...
    SearchResultVote.objects.create(
        user=verified_user, url="https://valid-url.com/path", query=query, vote_type="upvote"
    )
