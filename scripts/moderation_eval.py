...
def main() -> int:
    ...
    parser.add_argument(
        "--derived", type=int, default=0, help="Blocklist rows per under-represented reason (0 = skip the fetch)"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            f"Overwrite the committed warm start in {ARTIFACT_DIR}. Needed after a change to "
    ...
    print(f"real rows: {len(rows)} ({sum(row.rejected for row in rows)} rejected)")

    if not options.no_seed:
        rows += seed_rows()
    if options.derived:
        rows += derived_rows(["OFFENSIVE"], options.derived, exclude={row.domain for row in rows})
